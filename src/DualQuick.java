...
//		sorted = sort(toSort);
	...
	private static int[][] sort(int[][] toSort) {
        ...
        dualPivotQuickSort(sortMe, 0, count - 1);
	...
	private static void dualPivotQuickSort(long[] array, int lowIndex, int highIndex) {
		...
		int lesserIndex = lowIndex + 1;
		int greaterIndex = highIndex - 1;
		int i = lesserIndex;
		while (i <= greaterIndex) {
			if (array[i] < array[lowIndex]) {
				long temp = array[lesserIndex];
				array[lesserIndex++] = array[i];
				array[i++] = temp;
			} else if (array[highIndex] < array[i]) {
				long temp = array[greaterIndex];
				array[greaterIndex--] = array[i];
				array[i] = temp;
			} else {
				++i;
			}
		...
		long temp = array[--lesserIndex];
		array[lesserIndex] = array[lowIndex];
		array[lowIndex] = temp;
		temp = array[++greaterIndex];
		array[greaterIndex] = array[highIndex];
		array[highIndex] = temp;


		dualPivotQuickSort(array, lowIndex, lesserIndex - 1);
		if (array[lesserIndex] < array[greaterIndex]) {
			dualPivotQuickSort(array, lesserIndex + 1, greaterIndex - 1);
		}
		dualPivotQuickSort(array, greaterIndex + 1, highIndex);
	}
