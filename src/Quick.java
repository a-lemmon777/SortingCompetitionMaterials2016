...
//		sorted = sort(toSort);
	...
	private static void quickSort(long[] array, int lowIndex, int highIndex) {
		if (highIndex <= lowIndex) {
			return;
		}
		int splitIndex = partition(array, lowIndex, highIndex);
	...
	private static int partition(long[] array, int lowIndex, int highIndex) {
		...
		while (true) {
			while (array[++lesserIndex] < key) {
				if (lesserIndex == highIndex) {
					break;
				}
			}

			while (array[--greaterIndex] > key) {
				if (greaterIndex == lowIndex) {
					break;
				}
			}
			...
			array[lesserIndex] = array[greaterIndex];
			array[greaterIndex] = temp;
		}

		array[lowIndex] = array[greaterIndex];
		array[greaterIndex] = key;
		return greaterIndex;
	}
